        ...
        return consultarDni(dni)  // Obtienes la respuesta como JsonNode
                .flatMap(jsonNode -> {
                    try {
                        ...
                        // Verificar si el campo "success" está presente y no es nulo
                        if (jsonNode.has("success") && !jsonNode.get("success").isNull()) {
                            boolean success = jsonNode.get("success").asBoolean();
                            System.out.println("Success: " + success); // Solo para depuración
                        }
